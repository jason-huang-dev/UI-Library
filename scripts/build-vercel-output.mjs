...
const nativeStorybookOutput = resolve(root, "apps/storybook-native/storybook-static");
...
if (!existsSync(nativeStorybookOutput)) {
  throw new Error(`Native Storybook output not found: ${nativeStorybookOutput}`);
}

rmSync(output, { force: true, recursive: true });
mkdirSync(resolve(output, "native"), { recursive: true });
mkdirSync(resolve(output, "native-web"), { recursive: true });

cpSync(storybookOutput, output, { recursive: true });
cpSync(nativeStorybookOutput, resolve(output, "native"), { recursive: true });
